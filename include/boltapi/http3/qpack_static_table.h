...
//
// Header-only, self-contained (no HTTP/2 / HPACK dependencies). Ported verbatim
// from the audited FasterAPI implementation
// (src/cpp/http/qpack/qpack_static_table.h) and reshaped to Bolt Tiger Style:
// bounded constexpr table, noexcept lookups, error returns (no exceptions),
...
// One entry of the QPACK static table.
struct QpackStaticEntry {
    std::string_view name;
    std::string_view value;
};

// The QPACK static table (RFC 9204 Appendix A). All lookups are O(table size)
// but the table is tiny and fixed (99 entries) so this is constant-bounded.
class QpackStaticTable {
public:
    static constexpr std::size_t kSize = 99;

    // Sentinel returned by find()/find_name() when nothing matches.
    ...
    static const QpackStaticEntry* get(std::size_t index) noexcept {
        ...
        assert(!entry->name.empty() && "static table name must be non-empty");
        return entry;
    }
...
private:
    // RFC 9204 Appendix A — full 99 entries (index 0..98).
    static constexpr QpackStaticEntry kEntries[kSize] = {
        {":authority", ""},                                     // 0
        {":path", "/"},                                         // 1
        {"age", "0"},                                           // 2
