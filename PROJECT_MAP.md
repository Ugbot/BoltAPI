...
- ⬜ **M1** — HTTP/1.1 + TLS + Bolt-native router + chained middleware + Bolt primitives
...
| `bolt::api::App` facade | ⬜ | thin, over CoroUnifiedServer; Gestalt-compatible surface + head_async |
...
| Test infrastructure (gtest + http fuzz) | ✅ | `cmake/BoltApiGTest.cmake` (`boltapi_add_test`) + `tests/util/http_fuzz.h` seeded request generator; 8/8 ctest green |
| Coroutine middleware onion | ⬜ | sync+async, next() short-circuit |
