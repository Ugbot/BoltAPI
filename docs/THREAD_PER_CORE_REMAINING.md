...
- [x] **D-part-2: dispatch frame arena.** New `http::dispatch_task<T>` (`include/boltapi/http/dispatch_task.h`) — a near-clone of `core::coro_task<T>` with `operator new`/`operator delete` on the promise that draw the coroutine frame from the existing `FrameArenaPool` (8 KiB × 256 arenas). Each frame is prefixed with a `DispatchFrameHeader { FrameArena* arena; }`; delete reads the header to release the arena (or `::operator delete` on the heap-fallback path when the pool's exhausted / frame's too big). `App::dispatch_coro_` returns `dispatch_task<CoroHttpResponse>`; `CoroHttpHandler` alias + `dispatch_http3` updated; the `co_await handler_(creq)` await sites in `coro_unified_server.cpp` are drop-in (awaitable surface matches). **Gate met:** `dispatch_alloc_test` measured allocs/req **2.00** (was 3.00); ceiling tightened to 2.0. MSVC 252/252; Linux/Clang epoll lanes (gateway 10/10 + WebRTC/UDP 96/96 + bench smoke failed=0) + io_uring lane (`seccomp=unconfined`) loadgen 64-conn failed=0 across /plaintext + /json (~22k + ~40k ok).
