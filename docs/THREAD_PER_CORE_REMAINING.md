...
- [x] **Dedicated `detached_task` type** (folded the deferred slice-C leak fix in here): `initial_suspend = suspend_always` (lazy) + `final_suspend = suspend_never` (compiler auto-destroys the frame at the end of the coroutine body — the canonical fire-and-forget pattern; different machinery from a final_awaiter that destroy()s in await_suspend, which tripped MSVC heap corruption in slice C's first attempt). `accept_loop` + `connection_handler_coro` now return `detached_task` and self-reclaim on completion. **Closes the pre-existing connection-handle leak.** Gate: MSVC 252/252 + Linux gateway 10/10 + WebRTC/UDP 96/96.
...
- [ ] **Known follow-up:** under unconfined+io_uring, 8/10 HttpClient tests pass but 2 SEGFAULT (`PostEchoByteExactRandomizedSizes`, `KeepAliveConnectionIsReusedAcrossBurst`) — sustained client-side load has a bug worth investigating (likely in the connect+pipelined send/recv path; simple GET/PUT and curl-driven traffic work). **Does not affect default CI** (Docker default seccomp → epoll fallback). To investigate: standalone repro with single client doing keep-alive POST burst, ASAN under unconfined, or strace the SQEs.
